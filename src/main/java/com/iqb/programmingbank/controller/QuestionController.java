...
import com.iqb.programmingbank.manager.CounterManager;
import com.iqb.programmingbank.model.dto.question.*;
import com.iqb.programmingbank.model.dto.question.*;
...
public class QuestionController {
    ...
    public BaseResponse<Long> addQuestion(@RequestBody QuestionAddRequest questionAddRequest, HttpServletRequest request) {
        ...
        //如果数据库存在题目 报错
        ThrowUtils.throwIf(
                questionService.query().eq("title",questionAddRequest.getTitle()).count()!=0,
                ErrorCode.EXISITING_ERROR);
    ...
    private void crawlerDetect(long loginUserId) {
        ...
        if (count > BAN_COUNT) {
            ...
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR, "access too many times");
        }
        // 是否告警
        if (count == WARN_COUNT) {
            // 可以改为向管理员发送邮件通知
            throw new BusinessException(110, "warning: access too frequent");
        }
    }
    ...
    public BaseResponse<Page<Question>> listQuestionByPage(@RequestBody QuestionQueryRequest questionQueryRequest,HttpServletRequest request) {
        ...
        //如果是管理员 用管理员的查询逻辑 不是用用户的查询逻辑 差别在于用户只能查询自己创建的题目
        if("admin".equals(loginUser.getUserRole())){
            // 查询数据库
            questionPage = questionService.listQuestionByPage(questionQueryRequest);
        }else{
            questionPage = questionService.listQuestionForUserByPage(questionQueryRequest,loginUser.getId());
        }
   ...
   // @SaCheckRole(UserConstant.ADMIN_ROLE)
    ...
    public BaseResponse<Boolean> aiGenerateQuestions(@RequestBody QuestionAIGenerateRequest questionAIGenerateRequest, HttpServletRequest request) {
        ...
        // 校验参数
        ThrowUtils.throwIf(StrUtil.isBlank(questionType), ErrorCode.PARAMS_ERROR, "question type cannot be empty");
        ThrowUtils.throwIf(number <= 0, ErrorCode.PARAMS_ERROR, "question number must be greater than 0");
        // 获取当前登录用户
        User loginUser = userService.getLoginUser(request);
        // 调用 AI 生成题目服务
